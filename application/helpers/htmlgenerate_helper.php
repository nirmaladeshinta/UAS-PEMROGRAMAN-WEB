...
if (! function_exists('generatehtml')) {
    ...
    function buatcombo_order_by($nama, $table, $class, $field, $pk, $kondisi, $tags, $orderField, $orderKon)
    {
        $CI = &get_instance();
        $CI->load->model('Mcrud');
        if (empty($tags)) {
        ...
        if ($kondisi == null) {
            $data = $CI->Mcrud->getAllOrder($table, $orderField, $orderKon)->result();
        } else {
            $data = $CI->db->order_by($orderField, $orderKon)->get_where($table, $kondisi)->result();
        }
        echo "<div class='$class'><select name='" . $nama . "'  class='form-control' $tagtemp>";
        foreach ($data as $r) {
            echo " <option value=" . $r->$pk . ">" . strtoupper($r->$field) . "</option>";
